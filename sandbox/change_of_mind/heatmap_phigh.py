...
# %%
def plot_heatmap_phigh(probabilities, counts=None, n_rows=10, n_cols=10, difference=False,
                        cmap_name='seismic_r', diff_cmap_name='PiYG', show_counts=True):
    ...
    if show_counts and counts is not None:
        for row in range(n_rows):
            for col in range(n_cols):
                ...
                if np.isnan(probabilities[row, col]):
                    continue

                x = x_min + (col + 0.5) * grid_width
                y = y_min + (row + 0.5) * grid_height

                p = probabilities[row, col]

                #text_color = 'white' if (p > 0.7 or p < 0.3) else 'black'
                text_color = 'white' if (p > 0.8) else 'black'

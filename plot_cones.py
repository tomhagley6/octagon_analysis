...
def plot_cones(ax, trial_list=None, trial_index=0, trial=None):
  ...
  for i in range(num_players):
        ...
        cone_base_left = (x + 2*diameter * np.cos(cone_angle_left), y + 2*diameter * np.sin(cone_angle_left))
        cone_base_right = (x + 2*diameter * np.cos(cone_angle_right), y + 2*diameter * np.sin(cone_angle_right))

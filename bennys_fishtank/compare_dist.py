...
def direct_distance(this_trial, alcove_center, trial_index):
  ...
  wall_triggered = this_trial.loc[trigger_event.index, 'data.wallTriggered'].values[0]
  ...
  for i in range(num_players):
    coordinates.extend((globals.PLAYER_LOC_DICT[i]['xloc'], globals.PLAYER_LOC_DICT[i]['yloc'])) 
  
  coordinates = {label: this_trial[label].values[slice_onset_idx:] for label in coordinates}
